document.addEventListener("DOMContentLoaded", function () {
  ...
  function checkPasswordStrength() {
    ...
    if (
      ...
      validationResult4.isValid &&
      validationResult5.isValid
    ) {
  ...
  function updateSignupButtonState() {
    ...
    const isPasswordValid = !resultElement.classList.contains("invalid");
    const isEmailEmpty = emailInput.value.trim() === "";
    const isPasswordEmpty = passwordInput.value.trim() === "";

    // Update: Check email validation here
    const emailValidationResult = dfa.validateString(emailInput.value.trim());
    const isEmailValidationError = !emailValidationResult.isValid;

    // Enable the button only if both email and password are valid and not empty
